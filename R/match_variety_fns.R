...
return.matchgroups <- function(df_any_match, is_blends = FALSE){
  ...
  if (!is_blends & all(c("db_id", "var_id") %in% names(match_list[["match"]]))){
    ...
    if (nrow(test_multmatch > 0)){
      warning(paste("Warning: var_id matches with more than db_id. Please reconcile potential error in database.  Use check.matches(result[[1]]) to see the varieties that match more than one database name.",
