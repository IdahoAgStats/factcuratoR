...
```
codebooks_all_db.csv also specifies whether a variable must conform to 
...
Just as the main codebook defines the columns that are present 
in the trial_data and trials_metadata, codebooks_all_db.csv also 
defines the columns that are present in the controlled vocabulary
codebooks.  For the example (Fig. 1), codebooks_all_db.csv also has information for the column
names in the cultivar, nursery, location, and crop_market_classes controlled
vocabular codebooks.
...
(validation function will check to ensure is.character())
